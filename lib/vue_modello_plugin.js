...
function mixApiToModule (api, mod, option, globalOption) {
  ...
  Object.assign(mod.actions, {
    ...
    [title] (context, parameters) {
      ...
      if (paraOption.filter) {
        parameters = paraOption.filter(parameters)
      }
      ...
      return api.send(parameters).then(function (result) {
        ...
        if (globalOption.onError) {
          globalOption.onError(error && error.error || error)
        }
        ...
        switch (error.type) {
          case 'parameters':
            ...
            var parametersInvlidError = new HyperApiClientParametersInvalid(error.error)

            if (globalOption.onParametersInvlid) {
              globalOption.onParametersInvlid(parametersInvlidError)
            }

            if (globalOption.suppressParametersInvalidError) {
              console.error(parametersInvlidError)
              break
            } else {
              throw parametersInvlidError
            }
          case 'response':
            context.commit(U_RES_ERROR, error.error)

            var resultInvalidError = new HyperApiClientResultInvalid(error.error)

            if (globalOption.onResultInvlid) {
              globalOption.onResultInvlid(resultInvalidError)
            }

            if (globalOption.suppressResultInvalidError) {
              console.error(resultInvalidError)
              break
            } else {
              throw resultInvalidError
            }
          case 'request':
            throw error.error
          default:
    ...
    modello.on('mixed', function (eachMod) {
      eachMod(function (mod) {
        ...
        for (let apiKey in hyperApi) {
          let apiOption = hyperApi[apiKey]
          let api = Client[apiKey]
          if (api) {
            mixApiToModule(api, mod, apiOption, option)
          } else {
